...
我们以**Python 与 Go**为主（AI 生态主要在 Python，基础设施与性能敏感服务可用 Go），构建一套企业级的、可私有化部署的方案。
...
*目标：安全地执行代码和调用 API。*
*   **代码沙箱**：**Docker 容器** (自研低成本)
    *   **核心痛点**：Agent 写代码（如 `os.system('rm -rf /')`）极度危险。
    *   **方案**：必须在隔离的 Docker 容器中运行代码，用完即焚。
...
*目标：监控与风控。*
*   **追踪与监控**：**LangFuse** (强烈推荐)
    *   开源、可自托管。能看到每一次 LLM 调用的耗时、Token 消耗、输入输出（默认脱敏），对调试 Agent 极其重要。
